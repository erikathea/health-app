class LabDetail < ActiveRecord::Base
  ...
  private
  def check_one_bound
    return '' if eval("#{self.value} #{self.normal_range}") #value is within normal_range

    normal_range_array = self.normal_range.split(' ')
    left_bound = BigDecimal.new(normal_range_array.last) * BigDecimal.new("0.9")
    ...
    return (left_bound..right_bound).include?(BigDecimal.new(self.value)) ? 'warning' : 'critical'
  end

  def check_both_bounds
    normal_value_range = eval("#{ self.normal_range.gsub('-', '..')}")
    return '' if normal_value_range.include?(BigDecimal.new(self.value)) #value is within normal_range
